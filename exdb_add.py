...
def on_search():
    ...
    if search_term:
        ...
        show_data()
        code_value.config(text=f"{next_id}")
        clear_form()
...
def select_image():
    global pimage  # ประกาศ pimage เป็น global
...
def on_add():
    ...
    if not pname:  # เช็คชื่อ
        messagebox.showwarning("Warning", "กรุณากรอกชื่อต้นไม้")
        return
    if ptype == "เลือกประเภท":  # เช็คประเภท
        messagebox.showwarning("Warning", "กรุณาเลือกประเภทต้นไม้")
        return
    if pimage is None:  # เช็คภาพ
        messagebox.showwarning("Warning", "กรุณาเลือกภาพต้นไม้")
        return
    ...
    # ดึง next_id ใหม่เพื่ออัปเดตรหัส
    global next_id
    next_id = get_next_id()
    code_value.config(text=f"{next_id}")  # อัปเดตค่าใน label
    clear_form()
...
def clear_form():
    ...
    image_button.config(text="เลือกภาพ")
